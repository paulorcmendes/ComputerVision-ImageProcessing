...
if not os.path.exists(edgesPath):
    os.makedirs(edgesPath)
if not os.path.exists(blurPath):
    os.makedirs(blurPath)
...
for file in files:
    ...
    if extension == '.jpg' or extension == '.png':
        ...
        blur,edges, markedImage = findMedidor(img)
        cv2.imwrite(markedPath+'/'+file, markedImage)
        cv2.imwrite(edgesPath+'/'+file, edges)
        cv2.imwrite(blurPath+'/'+file, blur)